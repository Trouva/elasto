...
var Elasto = {
    ...
    */
    ...
    removeFrom: function(resourceName){
        return new RemoveOperation(this.basePath, resourceName);
    },
    ...
    /**
     * Get Aliases
     * @param  {String} index index (optional)
     * @return {Object}          Promise
     */
    getAliases: function(index) {

        var url = [this.server(), '_aliases'].join('/');

        if (index) {
            url = [this.server(), index, '_aliases'].join('/');
        }
     ...
     * @return {Object} Promise
     */
    removeAlias: function(index, alias) {
        var url = [this.server(), '_aliases'].join('/');
        var query = {
            actions: [{
...
var CreateOperation = function(basePath, resourceName){
    ...
    /**
     * version Optionally set the version attribute of the document for external versioning
     *         and optimistic concurency control.
     * @param {Object} doc [document]
     */
    ...
    this.save = function(){
        ...
        return new Bluebird(function (resolve, reject) {
            request({
                ...
                if (err) return reject(err);
                if (_.isString(body)) return reject(body);
...
var Query = function (basePath, resourceName) {
    ...
    */
    ...
    /**
     * Matches documents using OR boolean operator on other queries
     * @return Query
     * @example
     * query.or({
     *     sales: true,
     ...
     * });
     */
    this.or = function(opts) {
        for(var field in opts) {
            var filter = { term : {} };
            filter.term[field] = opts[field];
            this.options.should.push(filter);
        }
        return this;
    };
    ...
    this._buildQuery = function(term) {
        ...
        if(_.keys(bool).length) {
            ...
            // If we have a search term, combine it with the filter
            if(term) {
                query.query.filtered.query = {
                    'query_string': {
                        query: term
        ...
        if (_.keys(options.facets).length) query.facets = options.facets;
    ...
    */
    ...
    this.byId = function(id){
        ...
        return new Bluebird(function (resolve, reject) {
            request({
                ...
                if (res.statusCode !== 200) {
                    err = new Error(body.error || "item does not exists in the index");
                    return reject(err);
                }
    ...
    this.remove = function () {
        ...
        return new Bluebird(function (resolve, reject) {
            ...
            if(!_.keys(query.term).length) return reject(new Error('No term for remove'));
    ...
    this._buildRemoveQuery = function () {
        ...
        if (options.must.length) {
            ...
            options.must.forEach(function (filter) {
...
* @param {string} resourceName - Resource name
*/
var RemoveOperation = function(basePath, resourceName){
    this.basePath = basePath;
    this.resourceName = resourceName;
    this.id = null;

    /**
     * byId Set document to remove by id
     * @param  {String} id id
     ...
     * version Optionally set the version attribute of the document for external versioning
     *         and optimistic concurency control.
     * @param {String} versionValue The version number of the document
     * @param {String('Internal' || 'External')} versionType (optional)
     * @param {String} force
     * @return {Object} operation
     */
    this.version = function(versionValue, versionType, force){
    ...
    /**
     * remove Removes the document from elasticsearch
     *         and uses the mongoid as the id on the index.
     * @return {Promise} Returns the response of the operation.
     */
    this.remove = function(){
        var url = [this.basePath, this.resourceName, this.id].join('/');
